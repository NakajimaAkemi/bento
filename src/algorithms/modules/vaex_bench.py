...
class VaexBench(AbstractAlgorithm):
    ...
    def read_csv(self, path, **kwargs):
        ...
        :param kwargs: extra arguments
        """
        return vx.from_csv_arrow(path, **kwargs).head(30000)
    ...
    def join(self, other, left_on=None, right_on=None, how='inner', **kwargs):
        ...
        **kwargs: additional parameters
        ...
        if type(left_on) == str:
            self.df_ = self.df_.join(other, left_on=left_on, right_on=right_on, how=how, inplace=True, **kwargs)
        if (type(left_on) == list) and (type(right_on) == list):
            ...
            self.df_ = self.df_.join(other, left_on='mergingcol', right_on='mergingcol', how=how, inplace=True, **kwargs)
    ...
    def replace(self, columns, to_replace, value, regex):
        ...
        """
        value_map = dict(zip(to_replace, value))


        for col in columns:
            self.df_[col] = self.df_[col].apply(lambda x : value_map.get(x,x))
    ...
    def to_csv(self, path=f"./pipeline_output/{name}_loan_output.csv", **kwargs):
        ...
        #print(len(self.df_))
        #print(self.df_[:1])
        #print(self.df_.get_column_names())
    ...
    def loc(self, column_list,row_condition=None,):
        ...
        if row_condition is None:
            ...
            self.df_ = self.df_[row_condition]
        else:
