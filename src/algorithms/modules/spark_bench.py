...
class SparkBench(AbstractAlgorithm):
    ...
    def read_csv(self, path, **kwargs):
        ...
        self.df_ = self.sparkSession.read.csv(path, header=True, inferSchema=True).limit(30000)  # Initiate the dataFrame
        return self.df_
    ...
    def replace(self, columns, to_replace, value, regex):
        ...
        ##    for old_value, new_value in zip(to_replace, value):
        ##        expr = sf.when(expr == old_value, new_value).otherwise(expr)
        ##    self.df_ = self.df_.withColumn(column_name, expr.cast("string"))

        lambda_string = "lambda x: "
        if type(value[0])==str:
            for old_value, new_value in zip(to_replace, value):
                lambda_string += f"'{new_value}' if x == {old_value} else "
        ...
        if type(value[0])==str:
            self.edit(columns, lambda_string, 'string')
        elif type(value[0])==int:
            self.edit(columns, lambda_string, 'int64')
    ...
    def to_csv(self, path="pipeline_output/spark_output.csv", **kwargs):
        ...
        #print(self.df_.count())
        #print(self.df_.limit(1).show())
    ...
    def loc(self, column_list, row_condition=None):
        if row_condition is not None:
            self.df_ = self.df_.filter(sf.expr(row_condition))
        
        if column_list is not None:
            self.df_ = self.df_.select(column_list)
